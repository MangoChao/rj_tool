...
<style>
    ...
    .row-label { width: 30px; font-size: 0.7em; color: #666; text-align: center; font-weight: 700; }
    ...
    .auto-copy-wrap { display: flex; align-items: center; justify-content: center; gap: 10px; font-size: .85em; color: #888; }
    .btn-group { display: flex; gap: 8px; padding-bottom: 20px; position: relative; z-index: 10; }
    button { flex: 1; padding: 15px; font-size: 1em; font-weight: 700; border: none; border-radius: 8px; background: var(--my-green); color: #fff; cursor: pointer; -webkit-tap-highlight-color: transparent; }
    .btn-danger { background: #333; color: #999; }
    .fixed-footer { color: var(--accent); font-size: 14px; font-weight: 700; text-align: center; padding: 5px 0 15px 0; user-select: none; }
    ...
    #toast { position: fixed; top: 20px; left: 50%; transform: translateX(-50%); background: var(--my-green); color: #fff; padding: 10px 25px; border-radius: 25px; opacity: 0; transition: .3s; z-index: 9999; font-size: 14px; font-weight: 700; pointer-events: none; box-shadow: 0 4px 15px rgba(0,0,0,0.5); }
    ...
    .room-input { background: #000; border: 1px solid #444; color: var(--accent); width: 85%; padding: 12px; border-radius: 8px; font-size: 1.3em; text-align: center; margin: 5px auto; outline: 0; display: block; letter-spacing: 2px; }
    .room-input:focus { border-color: var(--my-green); }
    .small-btn { padding: 6px 5px; font-size: .9em; background: #333; color: #ccc; border-radius: 6px; border: 1px solid #444; flex: 1; cursor: pointer; font-weight: 700; }
    .small-btn:active { background: #444; }
    .manual-text { color: #aaa; font-size: 1em; margin: 8px 0; line-height: 1.4; }
    .edit-label { font-size: .85em; color: #666; margin-bottom: 5px; display: block; }
    .blink { animation: blinker 1.5s linear infinite; color: var(--my-green); font-weight: 700; }
    @keyframes blinker { 50% { opacity: .3; } }
    .edit-btn { cursor: pointer; margin-right: 6px; vertical-align: middle; transition: .2s; display: inline-flex; align-items: center; justify-content: center; }
    .edit-btn:hover { opacity: .6; }
    .edit-btn svg { width: 18px; height: 18px; stroke: #fff; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
    ...
    .color-cell.active { border-color: #fff; opacity: 1; transform: scale(1.05); }
    .color-cell.disabled { cursor: not-allowed; border: none; opacity: 1; box-shadow: inset 0 0 10px rgba(0,0,0,0.5); }
</style>
`;
...
const HOME_VIEW_PART = `
    <div id="home-view" class="card hidden" style="margin-top: 5vh;">
        ...
        <div class="version-info" style="margin-top:20px;">v1.3.9 | 最後更新: 2026-03-27 12:20</div>
...
<script>
        ...
        for (let r = 10; r >= 1; r--) {
            ...
            for (let c = 1; c <= 4; c++) {
                ...
                cell.onclick = () => {
                    ...
                    if (next === 1) {
                        for (let i=1; i<=4; i++) {
                            if (i !== c && globalGridData[r+'_'+i] && globalGridData[r+'_'+i][myName] === 1) syncAction(r, i, 0);
                        }
                    }
                    syncAction(r, c, next);
                };
                ...
                cell.oncontextmenu = (e) => {
                    ...
                    if (now - lastRightClick < 400) {
                        socket.emit('grid-action', { room: currentRoomId, r, c, name: 'ALL_CLEAR', state: 0 });
                        lastRightClick = 0; return;
                    }
                    lastRightClick = now;
                    const cur = states[myName] || 0;
    ...
    function askLoadRecord() {
        ...
        okBtn.onclick = () => {
            ...
            processLoadRecord(val); overlay.style.display = 'none';
        };
    ...
    function processLoadRecord(code) { 
        ...
        for (let r = 1; r <= 10; r++) { 
            for (let c = 1; c <= 4; c++) { 
                const k = r + '_' + c; if (globalGridData[k] && globalGridData[k][myName] === 1) {
                    socket.emit('grid-action', { room: currentRoomId, r, c, name: myName, state: 0 });
                    delete globalGridData[k][myName];
                }
            } 
        } 
    ...
    function showBackupModal(mainMsg, code) { const c = document.getElementById('modal-content'); const b = document.getElementById('modal-btns'); const o = document.getElementById('modal-overlay'); const d = formatDisplayCode(code); c.innerHTML = mainMsg + '<br>當前紀錄：' + d + '<br><span style="font-size:0.7em; color:#666;">點擊複製</span>'; c.onclick = () => copyText(d); b.innerHTML = ''; const r = document.createElement('button'); r.innerText = '嘗試重連'; r.onclick = () => { o.style.display = 'none'; initAction('join', currentRoomId, true); }; const h = document.createElement('button'); h.className = 'btn-danger'; h.innerText = '回到首頁'; h.onclick = () => location.href='/'; b.appendChild(h); b.appendChild(r); o.style.display = 'flex'; }
...
io.on('connection', (socket) => {
    socket.on('join-room', (data = {}) => {
        ...
        if (member) {
            ...
            const assignedName = availableNames[Math.floor(Math.random() * availableNames.length)]; // 隨機選一個
            
            // 隨機抽選顏色索引
            const usedColors = room.members.map(m => m.colorIndex);
            const availableColors = [0,1,2,3,4,5,6,7].filter(c => !usedColors.includes(c));
            const assignedColor = availableColors[Math.floor(Math.random() * availableColors.length)]; // 隨機選一個
