...
<script>
    ...
    const RAINBOW_COLORS = ['#ff4d4d', '#2ecc71', '#3498db', '#ff85a1', '#f1c40f', '#9b59b6', '#e67e22', '#1abc9c'];
    ...
    function renderGridStructure() {
        ...
        for (let r = 10; r >= 1; r--) {
            ...
            for (let c = 1; c <= 4; c++) {
                ...
                cell.onclick = () => {
                    const key = r + '_' + c;
                    const cur = (globalGridData[key] && globalGridData[key][myName]) || 0;
                    let next = (cur === 1) ? 2 : 1;
                    if (next === 1) {
                        if (Object.keys(globalGridData[key] || {}).some(n => n != myName && globalGridData[key][n] === 1)) return;
                ...
                cell.oncontextmenu = (e) => {
                    ...
                    if (now - lastRightClick < 400) socket.emit('grid-action', { room: currentRoomId, r, c, name: 'ALL_CLEAR', state: 0 });
                    else syncAction(r, c, 0); lastRightClick = now;
                };
    ...
    function initAction(type, roomID = null, autoRestore = false) { 
        ...
        const savedUid = sessionStorage.getItem('rj_uid');
        socket.emit('join-room', { roomId: rId, uid: savedUid }); 
    ...
    function setupSocketListeners() {
        ...
        socket.on('room-joined', d => {
            hideLoader(); currentRoomId = d.roomId; myName = d.identityName; 
            sessionStorage.setItem('rj_uid', d.uid);
        ...
        socket.on('error-msg', m => { 
            hideLoader(); if (m.includes('解散') || m.includes('無效') || m.includes('已滿')) showBackupModal(m, lastValidCode);
            ...
            for (let c = 1; c <= 4; c++) {
                ...
                Object.keys(states).forEach(n => {
                    if (states[n] === 1) { 
                        ...
                        cell.querySelector('.val').innerText = n.slice(-2);
                    }
                    else if (n === myName && states[n] === 2) { cell.classList.add('mine-wrong'); cell.querySelector('.val').innerText = 'X'; }
                });
                if (Object.keys(states).some(n => states[n] === 1 || (n === myName && states[n] === 2))) { probDiv.innerText = ''; } 
                else {
    ...
    function processLoadRecord(code) { 
        const cCode = cleanCode(code); if (!cCode || cCode.length === 0) return;
...
io.on('connection', (socket) => {
    socket.on('join-room', (data = {}) => {
        ...
        if (member) {
            // 找到舊身分，僅更新 Socket ID 確保連線同步
            member.id = socket.id;
        } else if (room.members.length < 4) {
            const usedNames = room.members.map(m => m.name);
            const assignedName = ANIMAL_NAMES.find(n => !usedNames.includes(n));
            const usedColors = room.members.map(m => m.colorIndex);
            // 擴展尋找範圍到 0~7
            const assignedColor = [0,1,2,3,4,5,6,7].find(c => !usedColors.includes(c));

            member = { id: socket.id, uid: uid || Math.random().toString(36).substring(2, 15), name: assignedName, colorIndex: assignedColor };
        ...
        socket.join(roomId);
        // 回傳給客戶端：當前房號、分配到的名字、地圖狀態、以及最重要的 UID (給前端存入 sessionStorage)
...
server.listen(3000, () => console.log('羅密歐與茱麗葉小助手 Online.'));