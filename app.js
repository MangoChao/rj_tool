...
const HOME_VIEW_PART = `
    <div id="home-view" class="card hidden" style="margin-top: 5vh;">
        ...
        <div class="version-info">v1.2.6 | 最後更新: 2026-03-27 10:30</div>
        <div class="fixed-footer" style="padding-top: 15px;">Made by CC</div>
    </div>
`;

const ROOM_VIEW_PART = `
...
const JS_PART = `
    ...
    <script>
        ...
        function handleManualAction() {
            ...
            if (!/^[A-Z0-9]+$/.test(val)) { showToast("房號只能包含英文字母與數字", true); return; }
            if (val.length < 6 || val.length > 10) { showToast("房號長度需為 6-10 字元", true); return; }
            currentRoomId = val; initAction('join', val);
        }

        // --- 格式化工具 ---
        function formatDisplayCode(rawCode) {
            if (!rawCode || rawCode.length !== 10) return rawCode;
            return rawCode.substring(0, 5) + " " + rawCode.substring(5);
        }

        function cleanCode(val) {
            return val.replace(/\\s/g, ""); // 去除所有空格
        }
        ...
        function updateMyGreenCode() {
            ...
            lastValidCode = code; // 內部邏輯維持純數字
            
            const display = formatDisplayCode(code);
            if (document.getElementById('live-code')) {
                document.getElementById('live-code').innerText = display;
            }
            
            // --- 修正 1：自動複製也要帶空格 ---
            if (document.getElementById('auto-copy-toggle')?.checked) {
                copyTextSilently(display);
            }
            return code;
        }

        function copyMyCode() { 
            copyText(formatDisplayCode(lastValidCode)); 
        }
        ...
        function showBackupModal(mainMsg, code) {
            ...
            content.innerHTML = mainMsg + '<br>當前紀錄：' + display + '<br><span class="copy-hint" style="font-size:0.7em; color:#666;">點擊複製</span>';
            content.onclick = () => copyText(display);
        ...
        function askLoadRecord() {
            ...
            const btns = document.getElementById('modal-btns');
            const overlay = document.getElementById('modal-overlay');
            content.innerHTML = '<div style="font-size:0.9em; margin-bottom:10px;">請輸入或貼上紀錄數字</div>' +
                               '<input type="tel" id="load-record-input" class="record-input" maxlength="11" placeholder="00000 00000">';
            ...
            confirmBtn.onclick = () => {
                ...
                let cVal = cleanCode(val); // --- 修正 3：載入時自動適應空格 ---
                if (!/^\\d+$/.test(cVal)) { showToast("格式錯誤，僅限數字", true); return; }
                processLoadRecord(cVal.padEnd(10, '0')); 
        ...
        function copyText(val) {
            if (!val || cleanCode(val) === "0000000000") return;
            if (navigator.clipboard && navigator.clipboard.writeText) {
                navigator.clipboard.writeText(val).then(() => showToast("已複製紀錄"));
            } else {
                const ta = document.createElement("textarea"); ta.value = val; ta.style.position = "fixed"; ta.style.left = "-9999px";
                document.body.appendChild(ta); ta.select(); try { document.execCommand('copy'); showToast("已複製紀錄"); } catch (e) {} document.body.removeChild(ta);
            }
        }
        function copyTextSilently(val) { 
            if (val && cleanCode(val) !== "0000000000" && navigator.clipboard) {
...
io.on('connection', (socket) => {
    socket.on('join-room', (data = {}) => {
        ...
        if (!room) {
            if (rooms.size >= MAX_ROOMS) return socket.emit('error-msg', '目前伺服器房間數已達上限，請稍後再試。');
...
server.listen(3000, () => console.log('羅密歐與茱麗葉小助手 v1.2.6 Online.'));